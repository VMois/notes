...
struct flcliapi_t {
    zactor_t *pipe;         //  Pipe through to flcliapi agent
};
...
flcliapi_t * flcliapi_new (void)
{
    ...
    self->pipe = zactor_new(flcliapi_agent, NULL);
    return self;
}
...
int server_ping (server_t *self, zsock_t *socket)
{
    if (zclock_time() >= self->ping_at) {
        zmsg_t *ping = zmsg_new();
        std::cout << self->endpoint << std::endl;
...
int
server_tickless (server_t *self, uint64_t tickless)
{
...
typedef struct {
    zsock_t *pipe;                 //  Socket to talk back to application
    zpoller_t *poller;
    zsock_t *router;               //  Socket to talk to servers
...
agent_t *
agent_new (zsock_t *pipe)
{
    agent_t *self = (agent_t *) zmalloc (sizeof (agent_t));
    self->pipe = pipe;
    self->router = zsock_new(ZMQ_ROUTER);
    self->poller = zpoller_new(self->pipe);
    zpoller_add(self->poller, self->router);
...
agent_control_message (agent_t *self)
{
    ...
    if (streq (command, "CONNECT")) {
        ...
        int rc = zsock_connect(self->router, endpoint);
...
static void
flcliapi_agent (zsock_t *pipe, void *args)
{
    std::cout << "agent" << std::endl;
    agent_t *self = agent_new (pipe);
    zsock_signal (pipe, 0);
    ...
    while (1) {
        ...
        zlist_t *servers = zhash_keys (self->servers);
        server_t* item = (server_t*) zhash_first(self->servers);
        while(item) {
            server_tickless(item, tickless);
            item = (server_t*) zhash_next(self->servers);
        }

        zsock_t *which = (zsock_t *) zpoller_wait (self->poller, (tickless - zclock_time ()) * ZMQ_POLL_MSEC);
        /*int rc = zmq_poll (items, 2,
            (tickless - zclock_time ()) * ZMQ_POLL_MSEC);*/
        std::cout << (void*) which << std::endl;
        /*if (rc == -1) {
        ...
        item = (server_t*) zhash_first(self->servers);
        while(item) {
            server_ping(item, self->router);
            item = (server_t*) zhash_next(self->servers);
        }
    }
    std::cout << "agent destroy" << std::endl;
    agent_destroy (&self);
}
